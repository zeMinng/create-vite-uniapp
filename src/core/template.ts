...
export async function loadTemplateLayer(
  relativePath: string,
  ctx?: TemplateContext,
  ...
  const filesField = template.files ?? 'files'
  const filesRel =
    typeof filesField === 'function'
      ? (() => {
          if (!ctx) {
            throw new Error(
              `Template "${relativePath}" uses a dynamic files directory; pass TemplateContext to loadTemplateLayer`,
            )
          }
          return filesField(ctx)
        })()
      : filesField

  const filesDir = path.join(rootDir, filesRel)
  if (!fs.existsSync(filesDir)) {
    throw new Error(`Template files directory missing: ${relativePath}/${filesRel}`)
  }
